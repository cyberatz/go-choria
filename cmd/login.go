...
func (p *loginCommand) sign(user string, pass string) (signature string, pub string, err error) {
	...
	sig, err := choria.Ed25519Sign(priK, []byte(fmt.Sprintf("%s:%s", user, pass)))
	if err != nil {
